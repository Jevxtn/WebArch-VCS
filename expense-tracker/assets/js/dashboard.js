...
async function loadStatistics(filter = 'month') {
    try {
        ...
        if (data.success) {
            ...
            document.getElementById('transactionCount').textContent = data.recent.length;
...
function updateCategoryBreakdown(categories) {
    ...
    categories.forEach(category => {
        const percentage = ((parseFloat(category.total) / total) * 100).toFixed(1);
        const div = document.createElement('div');
        ...
        div.innerHTML = `
            <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                ...
                <span style="font-weight: 600;">$${parseFloat(category.total).toFixed(2)}</span>
            </div>
            <div style="width: 100%; background-color: var(--bg-secondary); border-radius: 4px; height: 8px; overflow: hidden;">
                <div style="background-color: ${category.color}; height: 100%; width: ${percentage}%;"></div>
            </div>
            <div style="font-size: 12px; color: var(--text-secondary); margin-top: 3px; text-align: right;">${percentage}%</div>
        `;
